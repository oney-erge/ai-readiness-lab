...
function Ensure-Uv{$uv=Resolve-Uv;if($uv){Write-Host "[AI Readiness Lab] Using uv at $uv" -ForegroundColor Cyan;return $uv};Write-Host "[AI Readiness Lab] Installing uv..." -ForegroundColor Cyan;$file=Join-Path $env:TEMP "airl-uv-$UvVersion.ps1";try{Save-InstallDownload -Url "https://astral.sh/uv/$UvVersion/install.ps1" -Destination $file -Label "uv download";& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $file}finally{Remove-Item -LiteralPath $file -Force -ErrorAction SilentlyContinue};$uv=Resolve-Uv;if(-not$uv){throw "uv installed but could not be located."};return $uv}
...
$uv=Ensure-Uv;Write-Host "[AI Readiness Lab] Preparing the Python environment..." -ForegroundColor Cyan;if(Test-Path $python){Write-Host "The project Python environment is already installed."}else{Invoke-InstallRetry "Python installation" {$output=& $uv python install 3.11 2>&1;if($LASTEXITCODE-ne 0){throw "uv python install failed: $($output -join [Environment]::NewLine)"};$output|Write-Host}};if(-not(Test-Path $python)){& $uv venv --python 3.11 .venv;if($LASTEXITCODE-ne 0){throw "uv venv exited with $LASTEXITCODE"}}
...
Write-Host "[AI Readiness Lab] Starting the local web UI..." -ForegroundColor Cyan
if(Test-Ready){Write-Host "AI Readiness Lab is already running at $url" -ForegroundColor Green;if(-not$NoBrowser){Start-Process $url};Complete-Install;exit 0}
Complete-Install
